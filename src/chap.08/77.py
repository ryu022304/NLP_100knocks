# 76の出力を受け取り，予測の正解率，正例に関する
# 適合率，再現率，F1スコアを求めるプログラムを作成せよ．
...
with open(opath+"sentiment.txt", encoding='cp1252') as f:
    ...
    for text in f:
        ...
        label = text[:2]
        if label == '+1' and y[0] == 1.0:
            tp += 1
        elif label == '+1' and y[0] == 0.0:
            fn += 1
        elif label == '-1' and y[0] == 1.0:
            fp += 1
        elif label == '-1' and y[0] == 0.0:
            tn += 1
    ...
    print('正解率: '+str(correct))
    print('適合率: '+str(precision))
    print('再現率: '+str(recall))
    print('f値 　: '+str(f_measure))
